...
There is no `POST /users` create payload in v1.5, so "client-supplied `id`/`createdAt`/`createdBy` returns `400`" has no surface. The seed/import writer owns all audit/identity fields by construction (it is the writer, not a request handler). No equivalent is needed. See Story 1.1 (`um-seed-*`).

**Transport specified 2026-09-02** (`epic-1-story-1-1-decisions.md`). The operator batch-import transport is now fixed: **`POST /users/import`**, `multipart/form-data`, one `file` part = the semicolon-delimited CSV, authorized by the existing `user-management:create` permission (HR-Admin-only in the seeded system — no new key or kernel seed). It carries **no** operator-supplied identity fields: `createdBy` is the ACM-0 root id by construction, `id`/`createdAt` are writer-owned. A structurally invalid file (no `file` part, non-CSV, header mismatch, unparseable) → `400`, nothing written; row-level errors → `200` with a per-row `skipped` / `errors[]` summary.
...
**Import source (2026-09-01).** Story 1.1's import reads the delivered semicolon-delimited timetracker export `docs/Accounts_template.csv` (header `FirstName;LastName;Email;Birthday;PositionId;PositionName;RegistrationDate;DepartmentId;DepartmentName;DismissedDate;IsDismissed;EmployeeType;TimeZone;CountryId;CountryCode;CountryName;CountryStateId;CountryStateName`). The file has **no employee-id column**, so the import is **keyed by the normalized `Email`** and normalization under this decision applies to the CSV's `Email` column. `ttId` (AD-13) has no source column → left `null`. A CSV row whose normalized `Email` equals `ROOT_WORK_EMAIL` **updates the ACM-0 root `User`** in place (DEC-UM-009). Column → `User` field mapping: `docs/test-cases/user-management/seed/README.md`, `_bmad-output/implementation-artifacts/user-management/spec-1-1-import-seeded-population.md`.

**Mapping OPEN items resolved 2026-09-02** (`epic-1-story-1-1-decisions.md` §2b/§2c/§4/§5): `IsDismissed`+`DismissedDate` → an `EmploymentStatus` row (`active`/`dismissed`), **never** `User.isActive` (which is set `true` for every imported row, dismissed included — it is the account/row-retention flag only); `DepartmentId`+`DepartmentName` → a `Department` created on import, identity = the `(externalId, name)` pair (`externalId` alone not unique — a divergent name is a second row), plus one `DepartmentMembership` per person; `PositionId` / `CountryCode` / `CountryStateName` / `EmployeeType` / `TimeZone` are not stored in Story 1.1; `city` has no source → `null`.
