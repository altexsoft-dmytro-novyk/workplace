...
- `user-management` — seeded users, org facts (reports-to, People Partner, and project membership), projects, departments, time-bounded employment status, durable Departure commands, the `UserEvents` career-timeline log (sole persistence owner, PM/AD-30), profile HTTP assembly, and the data/canEdit envelope (PM/AD-34). Users are loaded only by the idempotent seeded-population import (`POST /users/import` plus a deploy-script entrypoint sharing one writer), authorized by the existing `user-management:create` permission; there is **no per-employee registration flow and no `POST /users` single-create** (AD-16 / AD-21). Project membership rows are written only by TimeTracker sync (PM/AD-31); departments are `Department`/`DepartmentMembership` (PM/AD-35). PP assignment follows AD-19. Employment-state ownership is PM/AD-22. `UserEvents` writes are synchronous in the same transaction as the causing use case — no event bus or generic table-change listener in this iteration.
