...
- `user-management` — seeded users, org facts (reports-to, People Partner), project membership rows written only by TimeTracker sync (PM/AD-31), departments (`Department` / `UserDepartment`, PM/AD-35), time-bounded employment status, durable Departure commands, the `UserEvents` career-timeline log (sole persistence owner, PM/AD-30), profile HTTP assembly, and the data/canEdit envelope (PM/AD-34). Users are imported from the provided seeded population; there is no employee-registration/create capability (AD-16). PP assignment follows AD-19. `UserEvents` writes are synchronous in the same transaction as the causing use case — no event bus. Employment-state ownership is PM/AD-22.
