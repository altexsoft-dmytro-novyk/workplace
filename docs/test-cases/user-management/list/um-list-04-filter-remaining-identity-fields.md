...
**Trace:** epics.md Story 1.5 · FR-15 · requirements §4.1 ("Any field that exists on a profile can be used as a filter and as a column")
...
**Then** every returned record matches the filter value exactly, and no non-matching record is included. This closes the "**any** field on the profile is filterable" breadth FR-15/§4.1 claim for the remaining S1 columns — all S1 identity columns are now covered across `um-list-02`/`03`/`04`. Employment-status (dismissed) visibility and its authorized filter are proven separately by `um-list-05` and are not repeated here; the internal `ttId`/`isActive` flags are never public list filters (FR-15).
