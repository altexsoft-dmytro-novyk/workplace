...
- **U-19 normative coverage:** Weak/shape-only evidence for `TR-3.2-S10` (v1.5 §3.2 — S10 leaves; colleague dates only) — proves the read-only outcome, not the "colleague dates only" field-level restriction. Headless-facade proof, not API E2E. See `test-design-qa.md` § Normative coverage map.
