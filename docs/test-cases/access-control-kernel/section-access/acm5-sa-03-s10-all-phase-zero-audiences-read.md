# ACM5-SA-03 · S10 grants read to every Phase-0 audience

**Trace:**

- SPEC [CAP-5](../../../../_bmad-output/specs/spec-access-control-kernel-mvp/SPEC.md#capabilities) — S10 is `read` for every Phase-0 audience, never `write` or `none` for a confirmed valid audience.
...
- [access-control.md § The AccessControl facade](../../../architecture/access-control.md#the-accesscontrol-facade-ad-9) and [§ Matrix exceptions](../../../architecture/access-control.md#matrix-exceptions-33) — the facade returns a base decision; S10's colleague field subset is a downstream narrowing rule.
...
`AccessControlFacade.canAccessSection(viewerId, 'S10', targetEmployeeId)` for
each pair.

**Then** every promise resolves to `'read'`; none resolves to `'write'` or
`'none'`. The base decision does not expose or decide S10's dates-only
