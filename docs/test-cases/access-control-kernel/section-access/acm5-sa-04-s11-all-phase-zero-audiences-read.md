# ACM5-SA-04 · `profile:projects` grants read to every Phase-0 audience

**Trace:**

- SPEC [CAP-5](../../../../_bmad-output/specs/spec-access-control-kernel-mvp/SPEC.md#capabilities) — `profile:projects` is `read` for every Phase-0 audience, never `write` or `none` for a confirmed valid audience.
...
`AccessControlFacade.canAccessSection(viewerId, 'profile:projects', targetEmployeeId)` for
each pair.

**Then** every promise resolves to `'read'`; none resolves to `'write'` or
`'none'`. The base decision does not expose or decide `profile:projects`'s colleague
