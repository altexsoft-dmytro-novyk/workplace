...
- **U-19 normative coverage:** Weak/shape-only evidence for `TR-3.2-S01` (v1.5 §3.2 — S1 identity-card matrix) — proves the read/write/none outcome per audience, not the field-level matrix, photo exception, or "relationship fields not writable" clause. Headless-facade proof, not API E2E. See `test-design-qa.md` § Normative coverage map.
