...
*Platform Epic 1, Story 1.6 (`1-6-platform-test-design-refresh-v1-2-v1-5`). The 2026-08-29 body below is preserved as a point-in-time Create record; this supplement supersedes it where they conflict. Sources: `_bmad-output/planning-artifacts/platform/epics.md` (Story 1.6 AC), `_bmad-output/planning-artifacts/platform/changelog-traceability-matrix.md` §9, `docs/project-requirements.md` v1.5, `_bmad-output/test-artifacts/gate-decision.json`, `_bmad-output/planning-artifacts/architecture/architecture-people-management-ratification-2026-09-02/blockers.yaml` (rev 2026-09-03; "blockers.yaml" below). Full detail is in the twin supplements in `test-design-architecture-platform.md` and `test-design-qa-platform.md`.*

1. **SoT (AC bullet 1):** normative authority is `docs/project-requirements.md` **v1.5** (`:3-8`, Amendment 2026-09-02) — already the top of the v1.5 authority order in this handoff. No PRD v1.2 assumption is carried.
2. **PeopleForce (AC bullet 2):** **[GOOD TO HAVE]** optional prefill by candidate ID only; vacancies are platform-owned and not synced with PeopleForce (`docs/project-requirements.md:348,545-575`). Required PeopleForce behaviour = candidate-ID/link storage (`:361-364`). The "no PeopleForce vacancy source" lines in Epic guidance and Story-Level Mapping Rules already state this.
3. **TimeTracker is the only required integration (AC bullet 3):** §5.1 **[REQUIRED]** (`:530`); §0 "the one required integration" (`:18`). PeopleForce §5.2 is GOOD TO HAVE.
4. **§9 DoD negatives (AC bullet 3; matrix G11):** all five §9 DoD negatives (`docs/project-requirements.md:614-626`) must reach stories as explicit rows — (1) narrowed project-line cells (`TR-3.2-S05/S07/S11`, `TR-9-02`); (2) UI-creatable functional role, no deploy (`TR-2.3-01`, `TR-9-03`); (3) org-relationship + full-access grants non-self-assignable and journalled (`TR-2.1-06`, `TR-3.4-01`, `TR-9-04`); (4) named-recipient-only revocable shared link (`TR-4.8-AC`, `TR-9-05`); (5) timetracker over the seeded population (`TR-5.1-01..03`, `TR-9-06`). None is closed; each carries a named gate.
5. **PR-B-04 / OQ-117 re-gate (AC bullet 3):** OQ-117 is `status: closed` (2026-09-02, PM/AD-34 — user-management owns assembly, no new bounded context; `blockers.yaml:304-316`), implementation `partial`. Re-gate PR-B-04 in the PRODUCT/ARCH BLOCKED table and Story-Level Mapping to **DESIGN-CLOSED / IMPLEMENTATION-DEBT** (whole-row serialization debt, tracked with ARCH-ENV-01), not a discovery blocker. `OQ-114/115/116/105` and `CC-05` are likewise design-closed with implementation debt (flagged; full table re-gate deferred to the next Validate run). Still genuinely open: `CC-07` (P0), `OPERATIONAL-ENVELOPE`/PR-B-09 (P0), `TT-IDENTITY-01` (P0), `TT-PMDM-01` (P1), `OQ-PERM-01` (P1), `OQ-AC-EDIT` (P1).
6. **QUALITY-GATE-AC (P0) — open debt (AC bullet 4; matrix G10):** `_bmad-output/test-artifacts/gate-decision.json` (snapshot 2026-09-04T17:18:29Z, `contract_static`) reads `gate_status: FAIL`, but the breach is P1 only — `p1_status: NOT_MET` at 70% vs an 80% floor, sole cause the `MENTORSHIP` blocker (all 27 live mentorship criteria PARTIAL; no `src/mentorship/`, no `MentorshipPair` model). The functional P0 scope this AC governs is **met**: `p0_status: MET`, P0 128/128 (100%); `blockers.yaml:140-161` records `QUALITY-GATE-AC` `status: closed` (2026-09-02) with `ACM3-II-04/-II-05/-II-06` each independently Stage-2-approved (`ACM3-II-06` = inactive-identity fail-closed). Net: the ACM3-II-06 / P0 scope is closed, but the gate as a whole is **not PASS** — carried as open debt until a re-evaluated `gate-decision.json` shows `gate_status=PASS` with `p0_status=MET` and `critical_open=0`. Later snapshot `gate-decision-repo-2026-09-06.json` → `CONCERNS`, P0 still 100%/PASS, P1 81%; debt stands. Applies to PG-01 / PG-03.
7. **QUALITY-GATE-AC-NFR — ACM-9 500-target / ≤2s (AC bullet 5):** tracked separately from the functional gate (`blockers.yaml:200-202` — `gate-decision.json` "does not include this blocker"). `blockers.yaml:163-202` — `status: closed` (2026-09-02, `implementation_status: proven`) against final `_bmad-output/test-artifacts/performance/acm9-final-acm9-1788173458416-ff94a3e685d1.json` (`ACM9-MVP-v1`, `role: final`, `PASS`; 500 active targets × 32 gates; warm p95 11.603 ms, worst case 12.357 ms) vs baseline run `acm9-1788173258311-697e946d9f11` (file `_bmad-output/test-artifacts/performance/acm9-baseline-acm9-1788173258311-697e946d9f11.json`; `comparability: comparable`). Feeds PG-04 / `TR-7-03`; independent of item 6.
8. **Live gates use `TT-IDENTITY-01` / `TT-PMDM-01`, not `TIMETRACKER-CONTRACT` (AC bullet 6):** `TIMETRACKER-CONTRACT` is `superseded_by: [TT-IDENTITY-01, TT-PMDM-01]` (`blockers.yaml:120-138`). "PR-B-08" / "the timetracker contract" in this handoff = `TT-IDENTITY-01` (P0; `:594-615`) + `TT-PMDM-01` (P1; `:617-630`). `TR-2.1-07`, `TR-5.1-01..03`, `TR-7-04`, `TR-9-06`, PG-02, workflow sequence step 4 must cite those IDs.
9. **Evidence caveat — verbatim (AC bullet 7),** from `blockers.yaml:17-22`:
   ...
   Committing the contract is a separate owner decision (`ARCHITECTURE-RATIFICATION.md` evidence-baseline caveats, `:42-46`).
10. **171-file AC inventory deleted 2026-09-04 (AC bullet 8; matrix G9):** the `docs/test-cases/access-control/` Phase-1 draft suite (171 files, cited throughout this handoff and in the Artifact Inventory row) **was deleted 2026-09-04** (`docs/architecture/access-control.md:345`) — unapproved, never executed. **No AC scenario inventory currently exists.** Every "171 files" / "171 drafts" reference below now means "the withdrawn 171-draft suite; no current AC coverage — authoring requires a fresh AD-1 Stage-1 dispatch." `PG-01` stays unschedulable for that reason.
...
| Access Control Phase-1 suite | ~~`docs/test-cases/access-control/README.md` and 171 scenario files~~ | **Deleted 2026-09-04** (unapproved, never executed — `docs/architecture/access-control.md:345`). No AC scenario inventory currently exists; see supplement item 10. |
