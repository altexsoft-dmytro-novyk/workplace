...
| **U-2** | **Still open** | Formal Product Owner/Architect sign-off for `PR-S-01`/`PR-S-02`. Not granted, not inferred; AD-19/AD-20 being binding is a direction, not a sign-off. Task 2 changed nothing here. | §5.2, §15.2 |
...
| **U-18** | `docs/architecture/testing-strategy.md:84` still reads "Each scenario still stops for its own human approval before stage 2", which contradicts the same document's lines 25–38 removing stage approval on 2026-09-04. Which sentence does the document intend to keep? | Owner of `docs/architecture/testing-strategy.md` (Architect) | Found by Task 2's checkbox-5 recheck while re-reading the binding policy. **For this ledger the question is already settled** — ruling D-1 names lines 25–38 as the authority, and nothing here relies on line 84 — but a binding document contradicting itself is a defect its owner should resolve. Task 2 **did not edit the source**. (§15.0, §15.5) |
| **U-19** | Which of the **101** real access-control scenario files (`access-control-foundation/` 10 + `access-control-kernel/` 91) covers which of the **119** `plat:TR-*` normative rows? | Access Control owners + QA | Exposed by §5.6's re-derivation. The old pointer ("171 files under `docs/test-cases/access-control/`") was dead, and re-deriving the inventory does **not** re-establish the row-to-file mapping the dead pointer used to imply. No artifact at `76a7220` answers it, so "which normative rows have a scenario today" is currently unanswerable — and it must not be guessed from filenames. (§5.6) |
| **U-20** | Now that `PG-01`'s per-file-approval rationale is retired, what **recorded condition** makes access control schedulable, and who evaluates it? | Platform epic owner + Architect | Exposed by §5.6/§5.8. `PG-01` correctly stays **not schedulable**, but its replacement rationale is three currently-open blockers — a state that will change without anyone being obliged to notice. A gate whose closing condition is unstated cannot be closed honestly. (§5.8) |
...
| **U-23** | What is the current implemented-test count, and which of "340 e2e", "406 cases / 43 files" is right? | QA | Exposed by §8.1. The same source document states both figures on the same date, and `find test -name '*.e2e-spec.ts' \| wc -l` at the pinned backend commit returns **54** files, not 43. Until someone re-counts, implemented-test figures stay `unverified` and are excluded from the estimate. **Task 2 ran no suite and reports no pass rate.** (§8.1) |
| **U-24** *(number assigned 2026-09-10)* | Which harness measures the All Employees list ≤2-second requirement (contract **A** of §7.1)? It does not exist, and no binding document selects one. | Platform / DevOps + QA (the harness); Product Owner still owns U-3's threshold statistic | Exposed by §4.3c's re-adjudication of `legacy-um:TD-UM-NFR-PERF-01`, and **cited in that row's prose as "U-20" before this number existed**. `k6` is named only by the superseded test-design set, by stock BMad skill templates, and by one planning document marked `Status: Planned` — in no binding architecture document, no requirement, no scenario and no `package.json` (§6, F-17). The evidence owner `PMC-E1-S1.9` specifies dataset, statistics, measured shape and failure semantics and **names no harness**. The repository's own successor scenario `list/um-list-12-perf-nfr2-stage2-note.md` proposes "a dedicated perf/load spec (tagged `@perf`, seeded to 500+ rows)" — a **proposal inside a scenario document, not a decision**. ACM-9 and P6 do not discharge it: different subjects (§15.3). Recorded as **UNDECIDED**; not invented here. (§4.3c, §5.8, §7.1, §8.1 category 4, §15.3) |
| **U-25** *(number assigned 2026-09-10)* | Blocker `QUALITY-GATE-AC-NFR` is recorded **closed** on ACM-9 facade-resolver evidence, while `PMC-E1-S1.9` routes the *directory-list* evidence into that same gate name. Which subject does the gate govern, and does the list requirement need its own gate identity? | Access Control + Quality Engineering (the blocker's named owner) + the platform-capabilities epic owner | Exposed by §5.8's `plat:PG-04` adjudication, and **cited in that row's prose as "U-19" before this number existed**. Evidence in §6, F-15: the blocker's closure condition is entirely ACM9-MVP-v1 (500 requested active targets, warm p95 and worst case), and its `closure_evidence` names an ACM-9 FINAL artifact — contract **B**. `PMC-E1-S1.9`'s acceptance criteria evaluate the same gate name for contract **A**, which has no measurement at all. A closed resolver gate does **not** discharge the list gate. This ledger neither reopens, closes nor renames the blocker, and **does not disturb the standing decision that the ACM-9 CI job stays informational**. (§5.8, §7.1, §15.3) |
...
| **U-2** | Formal Product Owner/Architect sign-off for `PR-S-01 / CC-04` (People Partner) and `PR-S-02 / CC-06` (scheduled departure). | Product Owner + Architect | §5.2 | Did not grant, imply, or infer sign-off. A binding architecture direction (AD-19/AD-20) is not a sign-off. |
...
| **U-12** | Test-file location conventions: backend unit specs beside the source (`*.spec.ts` under `src/`) or under `test/unit/`; frontend co-located `*.test.tsx` or a `test/` tree; the second vitest config and `@testing-library/react`. | DEV | `um-epic:S5#entry-criteria`, `fe-epic:S6#entry-criteria`, §5.1 | Did not choose. These are epic-plan entry criteria, unticked. |
...
whose scope now covers both locations; the source was **not** edited.
