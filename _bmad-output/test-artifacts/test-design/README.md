...
>   validated, except where their own checkpoint and epic row record a later explicit decision.
>   A new document never inherits an old document's human approval or validation PASS.
...
| Platform — Access Control Kernel MVP | `PLAT-E3` · `planning-artifacts/platform/epics.md` `## Epic 3: Access Control Kernel MVP` | `test-design-epic-platform-3.md` | `test-design-progress-epic-platform-3.md` (`epic-platform-3`) | **WRITTEN (2026-09-12).** Created under the one-plan-per-active-epic policy. Covers headless kernel risks and test design for ACM-0/1/2/3/4/5/8/9; it neither asserts execution evidence nor changes the User Management adoption boundary. It records the later E4 three-versus-six `hr-admin` permission-set drift as an open cross-epic dependency rather than resolving it — the `PLAT-E4` validation below records that reconciliation as **unreceived** at E4. Approval **granted by the requester (2026-09-12)**, scoped in the plan's Approval section to the design itself — risk register, NFR planning, coverage obligations, AC traceability, priority model, estimates — and explicitly not to coverage, gate result, execution evidence or release readiness. Approval and validation are separate states (contract §5) and neither confers the other. Validation **CONCERNS (2026-09-12, third run)** — [`test-design-validation-report-epic-platform-3.md`](../test-design-validation-report-epic-platform-3.md). **Verdict history on this one path, all 2026-09-12:** CONCERNS (pre-correction text) → PASS (**self-validation** by the authoring session) → CONCERNS (**independent of authorship**, superseding that PASS) → CONCERNS (this run, after an Edit). Each supersedes its predecessor and inherits nothing from it (contract §5). **The design substance has been checked three times and holds**, including a full independent re-count of all 49 canonical acceptance criteria and re-verification of every citation the earlier audit had found fabricated. The Edit run's tracing of the stale interim `ACCESS_CONTROL_PORT` binding past the scenario cards to its root in **SPEC CAP-6** (`spec-access-control-kernel-mvp/SPEC.md:150`) was re-checked against primary sources and holds, as did its ordering constraint (CAP-6 must be amended before the cards, which trace to it), its Stage-1 approval constraint (`approvals.yaml`, `ACM-8-scenarios`, approved 2026-08-31), and its structural finding that **there is no `access-control` planning domain**, so the UMAC-1 follow-up recorded 2026-09-01 had no backlog to be scheduled against. C-2's owner is recorded as the **in-flight** `ACM-8R-scenarios` story in `spec-access-control-kernel-mvp/stories.yaml` (another session's uncommitted work), superseding an earlier DEPT-4 suggestion. **Four earlier concerns and one WARN verified closed.** **Two concerns remain, both narrow:** the checkpoint's `nextStep` frontmatter still routes C-2 to DEPT-4, contradicting its own body — the **third** time a correction has been applied to a body and left a metadata field behind; and Open item 12 presents a completed root-trace whose enumeration is short by two (`acm8-kc-01` and `acm8-kc-05` also carry the superseded expectation — seven surfaces, not five), though the in-flight story's scope already covers all five cards. Four WARNs, including that at `HEAD` the committed report still reads `verdict: PASS` while the committed plan and checkpoint record CONCERNS — a repository-state split that resolves on commit. Neither concern moves an obligation, risk, priority, threshold or estimate. Coverage: none asserted. |
...
above), **`epic-platform-3`** (verdict CONCERNS, 2026-09-12, third run — see the `PLAT-E3` row above) **and
