...
> any superseded file**.
>
> - **Platform (system-level) artifacts are approved and validated PASS (2026-09-11).** Each
>   validated scope projects the same verdict, date, and report path into its plan, progress
>   checkpoint, and Status row. The canonical validation report is the evidence record. A new
>   document never inherits an old document's human approval or validation PASS.
...
| --- | --- | --- | --- | --- |
| **Platform (system-level)** | **Not an epic.** Sources: `docs/project-requirements.md` v1.5 (normative) · `_bmad-output/planning-artifacts/prds/prd-people-management-2026-08-24/prd.md` + `addendum.md` · `…/architecture-people-management-2026-08-19/ARCHITECTURE-SPINE.md` (PM spine) · the binding rendered rules under `docs/architecture/` | `test-design-architecture.md` · `test-design-qa.md` · `test-design/people-management-handoff.md` | `test-design-progress-system.md` (`runScope: system-level`, `runKey: system`, `workflowStatus: generated`) | **WRITTEN.** Approval **granted 2026-09-11**. Validation **PASS (2026-09-13, fresh independent system Validate, supersedes the 2026-09-11 PASS)**. Coverage: none asserted. **`test-design-qa.md` was edited again 2026-09-13** — a System Edit (contract §4.3) registering `TR-3.2-SELF` for the Self audience-derivation rule (`ACF-TR-01`, decision option A by the user acting as QA + Architect), mapped to `ACF-AU-01` and kernel `ACM4R-MA-02`, closing the prior U-19 orphan finding for `ACF-AU-01`. See that file's own 2026-09-13 System Edit note. |
| **Platform validation** | — | `test-design-validation-report.md` | — | **PASS (2026-09-13, fresh independent system Validate, supersedes 2026-09-11 PASS).** System Validate (contract §4.5) evaluated the architecture/QA pair and literal handoff at the content hashes recorded in the report, including the 2026-09-13 `TR-3.2-SELF` registration. No epic validation may overwrite it. |
...
| **This index** | — | `test-design/README.md` | — | **CURRENT LIFECYCLE PROJECTION.** Synchronized with each selected plan, checkpoint, and validation report; checked by `npm run test:test-design-lifecycle`. |
...
| User Management — Employee Record Management | `UM-E1` · `### Epic 1` | `test-design-epic-user-management-1.md` | `…-epic-user-management-1.md` (`epic-user-management-1`) | **WRITTEN.** Largest recipient: import/seed obligations, identity-card projection (backend **and** frontend subsections), list pagination and filters, `legacy-um:R-006/007/011`, `um-epic:R-UM-06`, `fe-epic:R-FE-02`. Ungranted. Validation **PASS (2026-09-13, fresh independent Epic-Level validation)** — [`test-design-validation-report-epic-user-management-1.md`](../test-design-validation-report-epic-user-management-1.md). One non-blocking finding: the Obligation Trace table's §4.3c and §4.4 per-section sub-counts are mis-stated (preserve/replace transposed in §4.3c; §4.4 undercounts by one merge row) even though the table's own Total row is independently confirmed correct. U-12's merge status is also stale (`services/frontend` `main` already contains it). Coverage: none asserted; no gate or release-readiness claim. |
| User Management — Magic-Link Authentication | `UM-E2` · `### Epic 2` | `test-design-epic-user-management-2.md` | `…-epic-user-management-2.md` (`epic-user-management-2`) | **WRITTEN.** Holds `TD-UM-AUTH-01..06`, `legacy-um:R-009`, `fe-epic:R-FE-01` and the predicate half of `R-FE-06`. **Carries a draft decision: `DEC-UM-012` is proposed, not approved.** Ungranted. Validation **PASS (2026-09-13, fresh independent Epic-Level validation)** — [`test-design-validation-report-epic-user-management-2.md`](../test-design-validation-report-epic-user-management-2.md). Obligation-trace counts reconcile exactly against the migration ledger; three non-blocking findings are open (U-12's merge status is stale — `services/frontend` `main` already contains it; the U-22 gap citation is not re-verified against current `AccountMenu.tsx` code; one hedged naming note). Coverage: none asserted; no gate or release-readiness claim. |
...
| Platform — Platform Spec v1.5 Alignment | `PLAT-E1` · `planning-artifacts/platform/epics.md` `## Epic 1` | `test-design-epic-platform-1.md` | `…-epic-platform-1.md` (`epic-platform-1`) | **WRITTEN 2026-09-11.** First plan created under the active-epic policy (below). **Carries no transferred obligation — 0 ledger rows target it**; its 27 `plat-e1:AV-*` artifact-verification obligations are newly stated from the epic's own acceptance criteria and are **not test cases**. Documentation-alignment epic: evidence levels are `repository-audit` / `manual-review` only. Approval **granted by requester (2026-09-12)** in its checkpoint. Validation **PASS (2026-09-12)** — [`test-design-validation-report-epic-platform-1.md`](../test-design-validation-report-epic-platform-1.md) supersedes the pre-edit CONCERNS report after the plan corrected U-16, TimeTracker provenance, U-18 and immutable ACM-9 evidence paths, then revalidated its AC→AV traceability remediation. No achieved coverage is asserted. |
| Platform — Access Control Foundation | `PLAT-E2` · `planning-artifacts/platform/epics.md` `## Epic 2: Access Control Foundation` | `test-design-epic-platform-2.md` | `test-design-progress-epic-platform-2.md` (`epic-platform-2`) | **WRITTEN (2026-09-11); edited twice 2026-09-13.** Created under the one-plan-per-active-epic policy. Approval **granted by the requester, Anna Pikula (2026-09-12)** for the test design only. **First 2026-09-13 Edit** recorded same-day completion of `ACF-AU-R1`, `ACF-FC-05`, `ACF-RW-04`, `ACF-DOC-01` (docs/ half — the SPEC.md half remains open, owner AC + Architect), `ACF-SCOPE-01`, and `ACF-SCOPE-02`. **Second 2026-09-13 Edit** closed `ACF-TR-01`: the user, acting as QA + Architect, decided option A, registering `TR-3.2-SELF` in `test-design-qa.md` § Normative coverage map and mapping it to `ACF-AU-01` and kernel `ACM4R-MA-02`; see the plan's Correction Log. Validation **PASS (2026-09-13, fresh independent Epic-Level validation, supersedes the same-day prior PASS)** — [`test-design-validation-report-epic-platform-2.md`](../test-design-validation-report-epic-platform-2.md), which independently re-verified the `ACF-TR-01` closure against `test-design-qa.md` and both cited scenario files, in addition to re-confirming every completion claim from the first 2026-09-13 Edit. Open items: `ACF-NC-01`, `ACF-PERF-01`, `ACF-DOC-01`'s SPEC.md half, and `SEC-AUTH-01` closure adjudication (Architect + Security). Coverage: none asserted; no gate or release-readiness claim. |
| Platform — Access Control Kernel MVP | PLAT-E3 · planning-artifacts/platform/epics.md · Epic 3: Access Control Kernel MVP | test-design-epic-platform-3.md | test-design-progress-epic-platform-3.md (epic-platform-3) | **WRITTEN (2026-09-12).** Approval **granted by the requester (2026-09-12)** for the design only; no coverage, gate, execution-evidence, or release-readiness claim. Validation **PASS (2026-09-12, fresh independent Epic-Level validation)** — [`test-design-validation-report-epic-platform-3.md`](../test-design-validation-report-epic-platform-3.md). The prior execution-state and Open item 12 description concerns are closed against current primary sources. Four bounded WARNs remain informational (threshold disclosure, omitted test count, not-applicable Pact inventory, and one citation anchor). Coverage: none asserted. |
| Platform — Access Control Authorization Consolidation | `PLAT-E4` · `planning-artifacts/platform/epics.md` `## Epic 4: Access Control Authorization Consolidation` | `test-design-epic-platform-4.md` | `test-design-progress-epic-platform-4.md` (`epic-platform-4`) | **WRITTEN (2026-09-12); edited again 2026-09-13.** Canonical plan for the human section-key, audience-first authorization, root-bootstrap, and dev-seed evidence boundary. It records the residual `seeded-two-level` ACM-9 decision as an open P2 evidence question; it does not conflate it with DIRA1 or P6. Approval **granted by the requester, Anna Pikula (2026-09-12)** for the test design only. **2026-09-13 Edit** recorded same-day completion of E4-C03b/E4-C04c (the PM/AD-24 401/404/precedence denial oracle, on backend branch `feat/conflict-um-01-hidden-target-404`, commit `89ea674`+, **not yet merged to `services/backend` `main`**), E4-C04a (root's own card, both bootstrap shapes), E4-C04b (the remaining `hr-admin` feature routes), the E4-C02/E4-AV01 static legacy-identifier oracle, E4-C06 obligation (6), and GAP-2/E4-C09's closure (per `dept-epic.md`, 2026-09-12); also recorded the `s42a-op-05`/`s42a-op-06` test-file move and the resolution of two test-review HIGH findings (oversize file; a cross-`describe` ordering dependency). Validation **PASS (2026-09-13, fifth re-validation, first for the 2026-09-13 Edit)** — [`test-design-validation-report-epic-platform-4.md`](../test-design-validation-report-epic-platform-4.md), which independently re-verified every 2026-09-13 completion claim (two reproducible test runs — 47/47 and 37/37 e2e, 12/12 unit — and its own `git merge-base` check confirming the `CONFLICT-UM-01` fix branch is still unmerged). One non-blocking finding (H-1): the E4-C04d row's `s42a-op-06` citation undercounts (says "Tests 1–2", the file has four), a pre-existing inaccuracy not introduced by today's edit. The prior **PASS (2026-09-12, fourth re-validation, after the AF-3 decision)** — the 2026-09-12 edit named the shipped, PO-accepted `profile:timeline:write` deviation (R03/E4-C04d, DEPT-2 closure), added verification obligations E4-AV01/AV02 for the two previously-uncovered Story 4.1 acceptance criteria, added a full Acceptance-Criterion Traceability table, received and explicitly declined the `PLAT-E3`-routed three-versus-six `hr-admin` reconciliation (DEPT-2/DEPT-4 owner), reconciled R06's priority to medium/P2 everywhere, and mapped the PM/AD-24 401/404/precedence denial oracle (R08, E4-C03b/C04c) as an open P0 blocker on `CONFLICT-UM-01` — that blocker is now recorded closed 2026-09-12 in `blockers.yaml`, with evidence on the still-unmerged branch above. Two 2026-09-12 re-validations independently re-verified E4-AV02/AV03 against `git grep`, `sprint-status.yaml`, and `acm1r-fr-foundation.e2e-spec.ts:58-59`, and the third confirmed E4-AV03's corrected re-run grep pattern. **The fourth 2026-09-12 run followed the AF-3 decision** (PO + Architect Anna Pikula declined the `spec-4-2d` development-fixture journal exception): R07 was re-scored 4→2 (low) and moved to a new low-priority table; every risk/coverage/traceability/mitigation/dependency/interworking mention was reconciled; the dev seed now journals every edge in `assignManager`'s shape (`s42d-ds-07`, verified independently against `org-relationship.repository.ts` and re-run: 24/24 pass on `s42d-ds-dev-seed-spine.e2e-spec.ts`, 53/53 on related suites, still on the separate, still-unmerged `feat/plat-e4-dev-seed-journal` branch). One non-blocking finding from that run (G-1): the checkpoint's specific "23/24, one pre-existing failure" claim did not reproduce (24/24 both times) — recorded as a documentation-accuracy note, not a defect. Coverage: none asserted; no gate or release-readiness claim. |
...
schedule. **Six exist:** `epic-platform-1` (PASS, 2026-09-12), `epic-platform-2` (PASS, 2026-09-13,
second same-day fresh independent validation, supersedes an earlier same-day PASS which itself
superseded a 2026-09-12 PASS at the same path; independently re-verified the `ACF-TR-01` closure
— `TR-3.2-SELF` registered, option A — plus the six prior same-day completion claims and the
remaining explicitly-open items),
`epic-platform-3` (PASS, 2026-09-12, fresh independent validation),
`epic-platform-4` (PASS, 2026-09-13, fifth re-validation, first for the same-day Edit that closed
the E4-C03b/C04c denial-oracle, E4-C04a/b, E4-AV01/C02 static-oracle, E4-C06(6), and GAP-2/E4-C09
evidence rows; one non-blocking finding H-1, a pre-existing test-count undercount; supersedes the
2026-09-12 fourth-run PASS, which itself carried one non-blocking documentation-accuracy finding
G-1), `epic-user-management-1` (PASS, 2026-09-13, fresh
independent validation; non-blocking Obligation Trace sub-count and U-12 findings), and
`epic-user-management-2` (PASS, 2026-09-13, fresh independent validation; non-blocking U-12 and
U-22-citation findings); see their rows above.
