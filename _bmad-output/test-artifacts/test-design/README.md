...
| Platform — Access Control Foundation | `PLAT-E2` · `planning-artifacts/platform/epics.md` `## Epic 2: Access Control Foundation` | `test-design-epic-platform-2.md` | `test-design-progress-epic-platform-2.md` (`epic-platform-2`) | **WRITTEN (2026-09-11).** Created under the one-plan-per-active-epic policy. Approval **granted by the requester, Anna Pikula (2026-09-12)** for the test design only. Validation **PASS (2026-09-12, fresh independent Epic-Level validation)** — [`test-design-validation-report-epic-platform-2.md`](../test-design-validation-report-epic-platform-2.md); its ungranted-approval wording is the historical state at validation time, before this separate human approval. ACF-NC-01's dynamic API-E2E plus repository-audit counts reconcile across its row, P1 total, summary, resource table, and grand total; P0–P3 criteria/purpose and PR/Nightly/Weekly structure are coherent; checkpoint Resume metadata uses the canonical five Create steps; and current QA §U-19 reflects retired per-file AD-1 approval and production-wired, UM-owned `GET /users/:id` semantics. Open scenario, audit, measurement, tracker, and security obligations remain planned rather than executed. Coverage: none asserted; no gate or release-readiness claim. |
...
| Platform — Access Control Authorization Consolidation | `PLAT-E4` · `planning-artifacts/platform/epics.md` `## Epic 4: Access Control Authorization Consolidation` | `test-design-epic-platform-4.md` | `test-design-progress-epic-platform-4.md` (`epic-platform-4`) | **WRITTEN (2026-09-12).** Canonical plan for the human section-key, audience-first authorization, root-bootstrap, and dev-seed evidence boundary. It records the residual `seeded-two-level` ACM-9 decision as an open P2 evidence question; it does not conflate it with DIRA1 or P6. Approval **granted by the requester, Anna Pikula (2026-09-12)** for the test design only. Validation **PASS (2026-09-12, fourth re-validation, after the AF-3 decision)** — [`test-design-validation-report-epic-platform-4.md`](../test-design-validation-report-epic-platform-4.md): the 2026-09-12 edit named the shipped, PO-accepted `profile:timeline:write` deviation (R03/E4-C04d, DEPT-2 closure), added verification obligations E4-AV01/AV02 for the two previously-uncovered Story 4.1 acceptance criteria, added a full Acceptance-Criterion Traceability table, received and explicitly declined the `PLAT-E3`-routed three-versus-six `hr-admin` reconciliation (DEPT-2/DEPT-4 owner), reconciled R06's priority to medium/P2 everywhere, and mapped the PM/AD-24 401/404/precedence denial oracle (R08, E4-C03b/C04c) as an open P0 blocker on `CONFLICT-UM-01`. Two re-validations independently re-verified E4-AV02/AV03 against `git grep`, `sprint-status.yaml`, and `acm1r-fr-foundation.e2e-spec.ts:58-59`, and the third confirmed E4-AV03's corrected re-run grep pattern. **This fourth run follows the 2026-09-12 AF-3 decision** (PO + Architect Anna Pikula declined the `spec-4-2d` development-fixture journal exception): R07 was re-scored 4→2 (low) and moved to a new low-priority table; every risk/coverage/traceability/mitigation/dependency/interworking mention was reconciled; the dev seed now journals every edge in `assignManager`'s shape (`s42d-ds-07`, verified independently against `org-relationship.repository.ts` and re-run: 24/24 pass on `s42d-ds-dev-seed-spine.e2e-spec.ts`, 53/53 on related suites). One non-blocking finding (G-1): the checkpoint's specific "23/24, one pre-existing failure" claim did not reproduce in this run (24/24 both times) — recorded as a documentation-accuracy note, not a defect. Coverage: none asserted. |
...
schedule. **Six exist:** `epic-platform-1` (PASS, 2026-09-12), `epic-platform-2` (CONCERNS,
2026-09-11), `epic-platform-3` (PASS, 2026-09-12, fresh independent validation),
`epic-platform-4` (PASS, 2026-09-12, fourth re-validation, after the AF-3 decision; one non-blocking documentation-accuracy finding G-1), `epic-user-management-1` (PASS, 2026-09-13, fresh
