...
| **`PR-B-08`** — timetracker contract | **`TIMETRACKER-CONTRACT` is superseded**, by **`TT-IDENTITY-01`** (P0 open) and **`TT-PMDM-01`** (P1 open) | `TT-IDENTITY-01`: project members arrive as `AccountTalentDto {email, dateStart, dateEnd}` with **no durable id**, email alone is insufficient by requirement, and PM/AD-13's `User.ttId` therefore has no population source. `TT-PMDM-01`: `projectManager` / `deliveryManager` are untyped strings while members in the same object are emails — joining an authorization edge on an unformatted display name is a **fail-open** risk. A third recorded gap: **no leaves endpoint** in the delivered contract (`TT-E1`'s subject). **Evidence provenance (corrected):** `docs/integrations/timetracker-external-api.json` is tracked at the migration baseline [`76a7220`](https://github.com/altexsoft-dmytro-novyk/workplace/blob/76a7220701ac6f16843dad8b303934f9a958b54c/docs/integrations/timetracker-external-api.json); the earlier “untracked at ratification pin” caveat is retired. File presence does not close either successor: both remain open on the substantive defects stated here. | Integration + Architect + Security |
...
| **U-16** | **Resolved 2026-09-12** — Story 1.6 is satisfied as a bounded documentation/evidence refresh. Its closure records historical P0 debt, Contract-B evidence, and TimeTracker provenance; it does not close a release gate, runtime dependency, or product requirement. | Platform epic owner | Story 1.6 completion record; `QUALITY-GATE-AC`, `TT-IDENTITY-01`, and `TT-PMDM-01` remain independently open as applicable |
