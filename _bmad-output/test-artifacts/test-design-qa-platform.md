...
*Platform Epic 1, Story 1.6 (`1-6-platform-test-design-refresh-v1-2-v1-5`). The 2026-08-29 body below is preserved as a point-in-time Create record; this supplement supersedes it where they conflict. Every change cites a source. Inputs: `_bmad-output/planning-artifacts/platform/epics.md` (Story 1.6 AC), `changelog-traceability-matrix.md` §9 (G9–G14), `docs/project-requirements.md` v1.5, `_bmad-output/test-artifacts/gate-decision.json`, `_bmad-output/planning-artifacts/architecture/architecture-people-management-ratification-2026-09-02/blockers.yaml` (rev 2026-09-03; "blockers.yaml" below). This supplement is mirrored in `test-design-architecture-platform.md`.*

**1. Source of truth (AC bullet 1).** Normative SoT is `docs/project-requirements.md` **v1.5** ("Version: 1.5", Amendment 2026-09-02 — `:3-8`). Child-ownership §, coverage map, and NFR plan already cite v1.5 as normative; no PRD v1.2 assumption is carried in this artifact. `docs/project-requirements-v1.2.md` is only the linked previous baseline.

**2. PeopleForce = optional prefill only (AC bullet 2).** PeopleForce is **[GOOD TO HAVE]** (`docs/project-requirements.md:545-575`, §5.2) — one prefill button, no synchronisation in either direction; vacancies are platform-owned and explicitly not synced with PeopleForce (`:348`, §4.7). Required PeopleForce behaviour is candidate-ID/link storage only (`:361-364`). Rows TR-4.7-01 ("no PeopleForce vacancy"), TR-4.7-03, TR-5.2-01/02, P3-PLAT-02, and "Not in Scope" already carry this; no "PF vacancies SoT / PF required" language is present.

**3. TimeTracker is the only required integration (AC bullet 3).** §5.1 is **[REQUIRED]** (`docs/project-requirements.md:530`); §0 (`:18`) — "the one required integration"; PeopleForce §5.2 is the only other integration, GOOD TO HAVE. TR-5.1-*, TR-9-06, PG-02 already treat timetracker as the sole required integration.
...
| # | §9 DoD negative | This artifact's rows / current status |
|---|---|---|
| 1 | Negative tests for narrowed project-line cells (PM/DM-via-project lose S2/S3; S5 = CV + certificates only; also unflagged S7 vs employee and PM; colleague whitelist + campaign-sender exception) | TR-3.2-S05/S07/S11, TR-3.3-02/04, TR-9-02 — **AC STAGE-1 / E2E DEPENDENCY**; Project-line cells deferred, gated by `TT-IDENTITY-01` (P0) + design-closed `OQ-116` (PM/AD-27) |
| 2 | New functional role creatable + grantable via UI, no deploy | TR-2.3-01/02, TR-9-03 — **E2E DEPENDENCY**; specific §2.3 keys not yet enumerated — `OQ-AC-EDIT` (P1 open: `user-management:edit`, `mentorship:assign` in no catalog), `OQ-PERM-01` (P1 open: default matrix unapproved) |
...
None is closed; each is an explicit row with a named gate. Exit-criteria and PG-05 already require every `—`/narrowed cell proven before release.

**3b. PR-B-04 / OQ-117 re-gate (AC bullet 3).** OQ-117 is `status: closed` (2026-09-02, design — PM/AD-34: user-management owns profile assembly, no new bounded context; `blockers.yaml:304-316`), `implementation_status: partial` ("Runtime still serializes whole User rows"). Re-gate PR-B-04 from **PRODUCT/ARCH BLOCKED** to **DESIGN-CLOSED / IMPLEMENTATION-DEBT**: Stage-1 profile/list design (TR-4.2-01, TR-3.1-01, P1-PLAT-01) may proceed against PM/AD-34; the open work is the audience-safe projection / whole-row-serialization envelope (transition debt with ARCH-ENV-01), not a bounded-context decision. Rows citing "PR-B-04 / OQ-117" as an open blocker (TR-4.2-01, DG-02) are stale to that extent.

**3c. Related stale blocker states (flagged, rows not rewritten).** Against `blockers.yaml` (rev 2026-09-03), also `status: closed` at design (2026-09-02) with implementation debt — not open discovery blockers: **OQ-114/PR-B-01** (`:268-285`), **OQ-115/PR-B-02** (`:287-302`), **OQ-116/PR-B-03** (`:680-694`), **OQ-105/PR-B-05** (`:697-711`), **CC-05/PR-B-06** (`:60-78`). Full re-gate of DG-02 and the blocked TR-*/P*-PLAT rows is beyond this doc-alignment story; flagged for the next Validate run. Of the PR-B set, genuinely still open: **PR-B-07 / CC-07 (P0)**, **PR-B-08 → TT-IDENTITY-01 (P0) + TT-PMDM-01 (P1)**, **PR-B-09 / OPERATIONAL-ENVELOPE (P0)**; plus `OQ-PERM-01` (P1) and `OQ-AC-EDIT` (P1) for the §2.3 permission keys. (`blockers.yaml` also carries open P0 `SEC-AUTH-01`, `CC-08`, `CC-09`, not modelled as PR-B rows here.)

**4. QUALITY-GATE-AC (P0) — current evaluated state, carried as open debt (AC bullet 4; matrix G10).** Per `_bmad-output/test-artifacts/gate-decision.json` (snapshot 2026-09-04T17:18:29Z, `collection_mode: contract_static`), the release gate reads `gate_status: FAIL`. The failure is confined to P1: `p1_status: NOT_MET` at 70% against an 80% floor, sole cause the `MENTORSHIP` blocker (severity high — all 27 live mentorship criteria PARTIAL, no `src/mentorship/` module and no `MentorshipPair` model). The functional P0 access-control scope this AC governs is **met** in the same snapshot: `p0_status: MET`, P0 coverage 128/128 (100%). `blockers.yaml:140-161` records `QUALITY-GATE-AC` as `status: closed` (2026-09-02) on the condition `gate_status=PASS, p0_status=MET, critical_open=0` with `ACM3-II-04` / `ACM3-II-05` / `ACM3-II-06` each carrying independently approved Stage-2 evidence — `ACM3-II-06` (inactive-identity fail-closed) among them; the prior FAIL record (evaluated 2026-08-31, uncovered ACM3-II-06) is historical. The gate as a whole is therefore **not PASS**: the ACM3-II-06 / P0 scope is closed, but overall `gate_status` stays `FAIL` in `gate-decision.json` on the unrelated P1 mentorship-coverage breach, and Story 1.6 carries that here as open debt until a re-evaluated `gate-decision.json` shows `gate_status=PASS` with `p0_status=MET` and `critical_open=0`. A later repo snapshot, `_bmad-output/test-artifacts/gate-decision-repo-2026-09-06.json`, moves the overall verdict to `CONCERNS` with P0 still 100%/PASS and P1 at 81%; it does not clear the debt. This feeds Exit-criteria bullet 1 and PG-01/PG-03.

**5. QUALITY-GATE-AC-NFR — ACM-9 500-target / ≤2s, tracked separately (AC bullet 5).** Independent of the functional `gate-decision.json` P0 gate — `blockers.yaml:200-202` records that `gate-decision.json` "does not include this blocker". `blockers.yaml:163-202` records `QUALITY-GATE-AC-NFR` as `status: closed` (2026-09-02, `implementation_status: proven`), closed against final artifact `_bmad-output/test-artifacts/performance/acm9-final-acm9-1788173458416-ff94a3e685d1.json` (protocol `ACM9-MVP-v1`, `role: final`, `status: PASS`, `stop_reason: completed`; 500 requested active targets across all 32 gates; warm p95 11.603 ms, worst case 12.357 ms — both inside the 2 s limit), compared in the same closure_evidence against baseline run `acm9-1788173258311-697e946d9f11` (file `_bmad-output/test-artifacts/performance/acm9-baseline-acm9-1788173258311-697e946d9f11.json`; baseline warm p95 12.702 ms; `comparability: comparable`). NFR-plan row "Performance — All Employees ≤2 seconds at 500+ records" and TR-7-03 / PG-04 are the planning surface; ACM-9 above is the closed evidence and neither gates nor is gated by the ACM3-II-06 functional coverage in item 4. Threshold source: `docs/project-requirements.md:596` ("the All Employees list with 500+ records … responds within 2 seconds, including permission resolution").

**6. Live/integration coverage gates use `TT-IDENTITY-01` and `TT-PMDM-01`, not `TIMETRACKER-CONTRACT` (AC bullet 6).** `TIMETRACKER-CONTRACT` is `status: superseded`, `superseded_by: [TT-IDENTITY-01, TT-PMDM-01]` (`blockers.yaml:120-138`). "PR-B-08" in this artifact resolves to: **`TT-IDENTITY-01`** (P0 — durable identity source for project members / AD-13 `ttId`; `blockers.yaml:594-615`) and **`TT-PMDM-01`** (P1 — resolvable identifier for `projectManager`/`deliveryManager`; `blockers.yaml:617-630`). Every live/integration gate — TR-2.1-04/07, TR-5.1-01..03, TR-9-06, P0-PLAT-05, P1-PLAT-05/07, PG-02, DG-04 — must cite `TT-IDENTITY-01` / `TT-PMDM-01`; `TIMETRACKER-CONTRACT` is historical only.
...
Committing the contract is a separate owner decision (`ARCHITECTURE-RATIFICATION.md` evidence-baseline caveats, `:42-46`). `TT-IDENTITY-01` / `TT-PMDM-01` evidence and any adapter Stage-1 (DG-04, PG-02) that cites the contract carry this caveat.

**8. The 171-file AC scenario inventory was deleted 2026-09-04 (AC bullet 8; matrix G9).** The Phase-1 `docs/test-cases/access-control/` draft suite — the "171 v1.5 Phase-1 draft scenario files" referenced in the coverage-state vocabulary, "Child ownership", "Dependencies & Test Blockers", "Entry Criteria", and every "AC STAGE-1 DRAFT" row — **was deleted on 2026-09-04** (`docs/architecture/access-control.md:345`: "the Phase-1 `docs/test-cases/access-control/` draft suite was deleted 2026-09-04 (171 unapproved, never-executed scenarios)"). Current state: **there is no authored AC scenario inventory.** Read "AC STAGE-1 DRAFT (171 files…)" as "no current AC scenario coverage exists; authoring any cell requires a fresh AD-1 Stage-1 dispatch." **PG-01 remains not schedulable** — now because no approved Stage-1 AC artifact exists at all, not because 171 drafts await per-file approval.
...
- **AC STAGE-1 DRAFT (171 files, per-file AD-1 approval pending)** — one or more Phase-1 files apply. This is not test coverage. *(1.6 supplement 2026-09-07: the 171-draft suite was **deleted 2026-09-04**, unapproved and never executed — `docs/architecture/access-control.md:345`. No AC scenario inventory currently exists; treat every "AC STAGE-1 DRAFT" row as "no current AC coverage — needs a fresh AD-1 Stage-1 dispatch." See supplement item 8.)*
...
| **PR-B-04 / OQ-117** | Profile bounded-context ownership | Owning context and API responsibility for S1–S16 projections and list contracts. *(Re-gated 2026-09-07 (1.6): DESIGN-CLOSED — PM/AD-34, user-management owns assembly, no new bounded context, `blockers.yaml:304-316`; open work is implementation debt, not a discovery blocker. See supplement item 3b.)* |
