...
- Project line, Department management, PP HR-line propagation, shared links, full-profile overlays, and policy-level `IN`. *(Deferral is by implementation phasing only; the v1.5 behavior of these items is normative and settled: department management is a **Reporting-line** relation, not provisional — v1.5 §2.1 relation 2 / §4.17, AD-10; project-line reach is **narrower** than Reporting — no S2, no S3, S5 = CV + certificates only — v1.5 §3.2 / §3.3.2; ordinary project membership is never an audience — PM/AD-27.)*
- A standalone end-user Access Control API or UI.

## Success signal

Once User Management supplies the integration contract, independently approved HTTP scenarios demonstrate correct Self, Reporting, direct PP, Colleague, and fail-closed outcomes through its route. The production implementation then supplies that behavior without changing User Management ownership. *(2026-09-01: UM's answer makes the Colleague and fail-closed cases resolver-set assertions rather than `GET /users/:id` status checks — see Open Questions.)*
...
- Which User Management-owned endpoint and exact response contract will consume the facade for the required HTTP E2E scenarios?
  ...
  **Superseded by PM/AD-24 (ratification 2026-09-02) — the empty-audience `403` above is historical 2026-09-01 approval evidence, not the live denial oracle.** The live oracle is `docs/architecture/access-control.md` §"Denial conventions" / PM/AD-24: invalid or inactive session `401`; missing or hidden-existence target `404`; visible resource but forbidden feature/action `403`; lists omit invisible rows; hidden-target `404` precedes mutation permission checks. Do not rewrite the 2026-09-01 decision record. The `ACF-AU-05` / `ACF-FC-01` / `ACF-FC-02` rework (`1-3a`) is **done**: the E2E now asserts resolver audience labels directly — `services/backend` `da7d1fa` (`test/access-control/audience-resolution.e2e-spec.ts`), 2026-09-06 — and the three scenario docs were brought in line 2026-09-07. The resolver behaviour was unchanged throughout.
