...
- **CAP-2** Phase 1 matrix cell scenarios
  - **intent:** Every §3.2 cell for Self, Reporting line, PP, and Colleague is asserted per read/write action, including flag-gated S7/S8 records and S16 visibility levels; Project line positives stay gated. **Reporting line and Project line are distinct columns (v1.5 §3.1/§3.2, AD-10)** — the Project line, when enabled, is **strictly narrower**: no S2, no S3, S5 = CV + certificates only, rest identical incl. S6 (§3.3.2). Those narrowed Project-line cells (S2/S3-absent, S5-narrowed) are **deferred to Platform Epic 6 / `AC-SECTION-MATRIX-01`**, not this Phase-1 stage-1 suite, which asserts only the Project-line *withhold* negatives under `matrix/project-line-gate/`.
...
- There is **no separate "unit" entity** (v1.5 §2.2, §4.17): *Unit Manager* is only the role name for a department's manager, and S1 uses the word "department". Scenarios reference the `Department` entity and its manager relation, never a "unit" object.

## Open Questions

- **OQ2 — RESOLVED 2026-09-02 (PM/AD-28).** Full-profile access is **not a §3.2 column.** It is a separate §2.4 overlay grant that can only raise a resolved section decision `none → read` (never to `write`, never over an unconfirmed/inactive target); Self is unaffected because Self already reads every section. There is no column-mapping question left. **No `matrix/full-profile-access/` scenarios are authored** (nor `full-profile/` — the two names refer to the same never-built folder); authoring them requires an AD-1 Stage-1 dispatch. (Draft prose for the *overlay behavior* — not matrix cells — lives under `docs/test-cases/access-control-kernel/full-profile-overlay/` for Platform Epic 4 / Story 4.2c, pending independent approval.) See `docs/architecture/access-control.md` §"Full-profile access overlay (§2.4)".
- **OQ3 — behavior CLOSED; only implementation open.** Department management is a **Reporting-line relation** and this is **normative, not provisional** (v1.5 §2.1 relation 2, §4.17; AD-10). It is no longer "provisional-only because it is not in the requirements." Still implementation-level: nested membership representation, indexed traversal plan, HR-boundary predicate for PP HR-line, and event/state sync details — each an AD-1 delivery decision.
- **OQ4 — behavior CLOSED; only implementation open.** Project-derived access withdraws **within 15 minutes** of assignment end and **after four hours** of failed sync (v1.5 §2.1, §5.1); the Project line is a **narrower** matrix than Reporting (no S2, no S3, S5 = CV + certificates only — §3.2/§3.3.2). Still implementation-level: timetracker assignment representation, freshness query seam, and partial/intermittent-sync semantics.
- **OQ6 — CLOSED (PM/AD-27, v1.5 §2.1).** A user may hold a project and its people without being PM/DM — and that grants **no Project audience**, full stop. Ordinary `Relationship type='project'` membership is never an audience; only an explicit PM/DM policy is. This is normative, not "deferred until product decides." Phase-1 withhold negatives cover it under `audience-derivation/` and `matrix/project-line-gate/`.
