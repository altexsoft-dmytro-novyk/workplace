...
> **UNAPPROVED DRAFT — do not dispatch.** No human has reviewed this SPEC. It is not a canonical contract, unlike the sibling specs it mirrors. Nothing here is approved scope, and no story in `stories.yaml` may be dispatched until a human accepts this document. Twelve open questions below are unresolved by the source material; four of them (OQ-3, OQ-4, OQ-5, OQ-11) block the Stage-1 scenario document that this slice depends on. Where the sources are silent, this SPEC records the silence — it does not fill it.
...
- **Re-authoring AC-FP-01, AC-FP-02, AC-FP-03** — three draft functional-permission scenarios already exist under `docs/test-cases/access-control/functional-permission/`, covering the dual gate, FR-is-not-audience, and HR-Admin-has-no-data-access. The Stage-1 work here extends the catalog's own surface and must stay consistent with those three rather than duplicate or contradict them. That folder's suite also explicitly defers "role-catalog management UI" scenarios, which is the hole this slice's Stage-1 document fills.
...
`database-schema.md` defines exactly three structures: `Policies`, `Permissions {id, title, description}`, and `UserPolicies {userId, policyId}`. There is **no join between `Policies` and `Permissions`**, and the ERD fragment shows only `User → UserPolicies → Policies`. But `PATCH /roles/:roleId/permissions` presupposes a many-to-many between a role and its permissions, and §2.3 requires a role to carry a mutable granted set. Either a fourth table is missing from `database-schema.md`, or a role's permission set is meant to live somewhere unstated. This must be decided and recorded in `database-schema.md` before any Stage-1 shape or migration. It is the single largest gap in this slice.
...
§2.3 lists prose names ("assign and end mentorships"). `database-schema.md` shows kebab-case examples (`'create-resourcing-requests'`, `'assign-mentors'` — note that the second is not a transcription of any §2.3 bullet). Nothing fixes what `isAllowed(userId, feature)` accepts: a `Permissions.title`, a `Permissions.id`, or a compile-time enum. A typed enum gives call-site safety but contradicts OQ-6 if permissions are runtime-creatable; a free string is extensible but makes a typo a silent denial. Decide together with OQ-6, and record the canonical spelling of all sixteen titles.
