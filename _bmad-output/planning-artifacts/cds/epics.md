...
- **PM/AD-35 / `DEPARTMENT-EDGE` — Nested Department schema** *(design resolved-approved 2026-09-02; **schema landed 2026-09-02, diverging from AD-35 — see below**)*: `Department {id, name, parentId, isHr}` with `UserDepartment {userId PK, departmentId}` — exactly one department per employee. AD-35's `Binds:` list names **"CDS key"** explicitly. **A `Department` table now exists** (migration `20260902001941_story_1_1_import_population`), but it diverges from AD-35 in three ways that bind this slice: membership is `DepartmentMembership` — **temporal and multi-valued**, not `UserDepartment {userId PK}`; there is **no `isHr` column**; and there is **no index on `parentId`** (the FK constraint alone does not create one in PostgreSQL). `DEPARTMENT-EDGE` names all three in its closure condition and stays open. The matrix lookup is buildable against the real schema, not against AD-35's.
...
- Every employee belongs to **one or more** departments — a person working across specialisations holds a current membership in each — and departments nest (requirements §4.17, **amended 2026-09-02**, was *exactly one*). **This corrects an error in this file's 2026-09-03 draft**, which carried the superseded wording. Consequence for SD-2: the matrix key resolves against a department the person currently belongs to, and a multi-department employee can resolve **more than one** candidate matrix entry. Which entry wins is a `[DERIVED]` decision recorded on Story 1.1. PM/AD-35's `UserDepartment {userId PK, departmentId}` shape encodes the superseded cardinality and is owed an amendment; the implemented schema is `DepartmentMembership`, temporal and multi-valued.
...
**[DERIVED]** A multi-department employee resolves **one matrix entry per current department**, and the section renders **all** of them rather than picking a winner — requirements §4.17 (amended 2026-09-02) makes multi-membership the deliberate case for people working across specialisations, and silently choosing one specialisation's matrix is the failure a reader could not detect. CDS-DR4's "displays the department and position it resolved through" is what makes several rows readable. Confirmer: Product Owner.
