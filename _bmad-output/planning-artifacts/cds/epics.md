...
2. **`AC-S9-S13`'s `blocks:` field omitted CDS — corrected 2026-09-03.** The blocker's `id`, and its `closure_condition` (*"a delivered AD-1 increment for S9-S13"*), both cover S12, while its `blocks:` list read only *"Career timeline writes, Mentorship profile projection and closure-note visibility"* — S9 and S13. This slice used `AC-S9-S13` on the strength of the ID and the closure condition and recorded that the `blocks:` list was owed a CDS entry. **`access-control` resolved it in this slice's favour: `blocks:` now names *CDS registry and CDS directory filters (S12)* explicitly.** No new gate ID was invented here.
