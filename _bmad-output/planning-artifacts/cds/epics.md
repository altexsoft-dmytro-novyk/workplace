...
- **PM/AD-35 / `DEPARTMENT-EDGE` — Nested Department schema** *(design resolved-approved 2026-09-02; implementation absent)*: `Department {id, name, parentId, isHr}` with `UserDepartment {userId PK, departmentId}` — exactly one department per employee. AD-35's `Binds:` list names **"CDS key"** explicitly. **No `Department` table exists.** The matrix lookup is unbuildable until this closes.
