...
- **Department** — every employee belongs to **one or more** departments (§4.17, amended 2026-09-02 — was "one department"); stored as an org fact — a `Department` nested entity + `DepartmentMembership` (`database-schema.md` §Project/Department) — never a free-text S1 field. The derived S1 "department" display is a **set**.
...
**`isActive` and `EmploymentStatus` are separate and neither is redundant (confirmed 2026-09-02).** `User.isActive` is the account/row-retention flag (`isActive = false` is effectively record deletion / hard retirement); it is never driven by employment and the seeded-population import sets it `true` for **every** row, dismissed included. `EmploymentStatus.status` is whether the person currently works here and drives directory-list visibility only. The Access Control kernel keeps joining `User.isActive` unchanged.

**Import-origin dismissals** (`database-schema.md` §EmploymentStatus). A `dismissed` row created by the import (`IsDismissed=1`) has **no `Departure`** — the CHECK no longer requires `sourceDepartureId` / `departureReason` on a `dismissed` row; both are `null`. There is no `source` discriminator column.

## Data Model — Department / DepartmentMembership (§4.17)

Binding shapes in `database-schema.md` §Project/Department. `Department { id uuidv7, name, externalId string nullable — the timetracker `DepartmentId`, **NOT unique on its own**, parentId FK? }` with `UNIQUE (externalId, name)` — identity is the pair, so the same `DepartmentId` arriving with a different name is a **distinct** department. `DepartmentMembership { id, userId, departmentId, validFrom date, validTo date? }` with `UNIQUE (userId, departmentId) WHERE validTo IS NULL`. An employee holds **one or more** current memberships (§4.17, amended 2026-09-02). The seeded-population import creates a `Department` on first sight of an `(externalId, name)` pair and writes **one** `DepartmentMembership` per person (the CSV carries one `DepartmentId` per row); department hierarchy (`parentId`) and Unit-Manager attachment have no CSV source and are assigned later. Department-derived access stays fail-closed until the Access-Control department-tree walk increment lands.
...
- **FR-4.** Employee population is imported from the delivered seeded timetracker list only (§4.17), through **`POST /users/import`** (multipart `file`, HR-Admin-only via the existing `user-management:create` permission) and a deploy/operator script entrypoint sharing one idempotent writer keyed by the normalized `Email`. Creating employees via a per-employee registration flow or `POST /users` is out of scope. `isActive` is the internal account/row-retention flag — the import sets it `true` for every row (dismissed included); it is not a public lifecycle field and not a substitute for S4 employment status.
