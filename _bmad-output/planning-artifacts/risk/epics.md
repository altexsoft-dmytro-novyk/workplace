...
|---|---|---|
| `SEC-AUTH-01` | **P0 open** | `isAllowedForTarget` returns `Boolean(userId)` — every target check passes — and the interim session resolver self-provisions a privileged `position: 'HR Admin'` account. Every risk read and write inherits both holes. Exposure is latent (backend not deployed), which sequences the fix, it does not downgrade it. *(2026-09-03 correct-course note: implementation evidence exists on the unmerged `dn-um-implementation` branch — see `blockers.yaml` `status_note`. Not yet merged or independently verified; this precondition stays open.)* |
