...
> **Denial-oracle annotation (Platform Story 1.3, 2026-09-07 — PM/AD-24 alignment, annotation only).** Everywhere the FR-16 / FR-17 text and the Epic 0 story below say a denial is a **`403` because the authenticated active viewer's audience over the target is empty**, that is **historical 2026-09-01 product-decision evidence, superseded by PM/AD-24 (ratification 2026-09-02)**. The live denial oracle is `docs/architecture/access-control.md` §"Denial conventions" / PM/AD-24: invalid or inactive session `401`; missing or hidden-existence target `404`; visible resource but forbidden feature/action `403`; lists omit invisible rows; hidden-target `404` precedes mutation permission checks. Regeneration of the affected UMAC scenarios/tests is an AD-1 dispatch (see `spec-user-management-access-control-adoption/SPEC.md` frontmatter `denial_oracle`). Do not rewrite the 2026-09-01 decision record. The `{ data, canEdit }` envelope, the S1-card projection, the audience set (self/reporting/pp/colleague all read S1), and the dual-gate write rule are unaffected by this annotation — only the empty-audience status code is.
...
**And** a **`401`** when the session does not resolve to an active `User` (the session layer's job; the interim resolver is lax, so such a request currently surfaces as `403` via the guard), and a **`403`** when an authenticated active viewer's audience over T is empty (T not an active `User`; no existence distinction — human product decision 2026-09-01, no "leak-free 404") *(empty-audience `403` / no-`404` = historical 2026-09-01 evidence, **superseded by PM/AD-24**, ratification 2026-09-02; live oracle is `401` / `404` / `403` per `access-control.md` §"Denial conventions"; affected UMAC scenarios/tests regenerate via AD-1 — annotation added Platform Story 1.3, 2026-09-07, do not rewrite the decision record)*
...
**Stage-1 sub-deliverables (AD-1):** the `um-seed-01`..`um-seed-13` suite under `docs/test-cases/user-management/seed/` — import success, no `POST /users` create path, bootstrap HR Admin + root-row reuse, Department create-on-import, `EmploymentStatus` mapping, birthday split, null source fields, idempotent re-import, file-level-`400` / row-level-`200` malformed handling, authz (`403`/`401`), deploy-script entrypoint, `joined_company` event atomicity. CAP-1 `um-reg-01`..`um-reg-15` are **retired / superseded** — do not translate to stage-2 for product create.

**Import source — `docs/Accounts_template.csv` (the file shape from TT).** A semicolon-delimited timetracker export. Header: `FirstName;LastName;Email;Birthday;PositionId;PositionName;RegistrationDate;DepartmentId;DepartmentName;DismissedDate;IsDismissed;EmployeeType;TimeZone;CountryId;CountryCode;CountryName;CountryStateId;CountryStateName`. The file has **no employee-id column**, so the import is **keyed by the normalized `Email`** and `ttId` (AD-13) has no source column → left `null`. Full column → `User` field mapping and the OPEN items in `docs/test-cases/user-management/seed/README.md` and `spec-1-1-import-seeded-population.md`. In brief: `Email`→`workEmail` (normalized, natural key); `FirstName`/`LastName`→`firstName`/`lastName`; `Birthday`→`birthDay`/`birthMonth` (year dropped; `NULL`→both null); `RegistrationDate`→`companyJoinDate`; `PositionName`→`position`; `CountryName`→`country`; `workPhone`/`city`/`photo`/`ttId` have **no source column** → `null`; `createdBy`→the ACM-0 root id. **Mapping resolved 2026-09-02** (`epic-1-story-1-1-decisions.md` — supersedes the earlier OPEN list): `PositionId` / `CountryCode` / `CountryStateName` / `EmployeeType` / `TimeZone` are **not stored** in Story 1.1; `city` has no source → `null`. `DepartmentName`+`DepartmentId` → a `Department` created on import, identity = the `(externalId, name)` pair (`externalId` alone **not** unique — a divergent name is a second row; `database-schema.md` §Project/Department), plus **one** `DepartmentMembership` per person. `IsDismissed`+`DismissedDate` → an `EmploymentStatus` row (`active`/`dismissed`), **never** `User.isActive` — the import sets `isActive = true` for every row, dismissed included (`isActive` is the account/row-retention flag only; the earlier "interim `isActive=false`" stopgap is withdrawn). Endpoint: **`POST /users/import`** (multipart, HR-Admin-only via the existing `user-management:create` key — no new permission or kernel seed) plus a deploy-script entrypoint; structurally invalid file → `400` nothing written, row errors → `200` with a per-row `skipped`/`errors[]` summary.
...
**Then** every seeded employee is addressable as an existing `User` with `isActive: true` — the import sets it `true` on **every** row, dismissed employees included (`isActive` is the account/row-retention flag; a dismissed employee is an `active` `User` row carrying an `EmploymentStatus` of `dismissed`)
...
**Implementation gate:** the atomic-journal half is blocked on **CC-07** (AD-19 Journal gate); department-derived access also depends on the still-open Department edge contract (spine Deferred) — until it lands, `department`-targeted policy rows contribute nothing (fail-closed). Scenario prose may proceed. **Note (2026-09-02):** §4.17 was amended so an employee holds **one or more** current departments (`DepartmentMembership`; `database-schema.md` §Project/Department) and `department_change` events are add/remove events. Whether "move" here means *replace the single membership* or *end one and add another* (and whether a person can be moved out of all departments) is an Epic 4 modelling question not settled by the Story 1.1 decisions — resolve it in this story's scenario pass.

**Acceptance Criteria:**

**Given** Alice holds a current membership in Department A and an authorized actor moves her to Department B
**When** the change commits
**Then** Alice holds a current membership in Department B (her Department A membership is ended per the move semantics this story fixes), Reporting-line access changes on the next request, a `department_change` career event is appended, and the journal records the before/after department
