...
- **Rule:** before adding any `Fake`/`Mock`/`Stub`/interim adapter anywhere in the codebase, name out loud which story, epic, or bounded context owns building the real thing.
  ...
  - **If the dependency belongs to a different, not-yet-built story, epic, or bounded context** (it is not this unit's job to build it): a fake/interim adapter here is legitimate and expected — this is AD-3's E2E-fake category, not a shortcut. The reference precedent is `MagicLinkDispatcherFake`: Story 1.1 triggers a magic-link dispatch as a side effect, but the real email-sending adapter is Epic 2's deliverable, so Story 1.1 fakes that one port and is still genuinely done.
