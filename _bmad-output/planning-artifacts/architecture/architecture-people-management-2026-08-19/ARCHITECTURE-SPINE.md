...
- **Amended 2026-09-04 (approval requirement removed):** the **ordering** above stands — scenario, then red Stage-2 test, then production written until it passes — but the **approval gate does not.** No stage waits on a developer sign-off, a dispatch may span more than one stage, and the 2026-08-26 extension above is withdrawn as a binding rule (kept here as the record of why it once existed). No new entries are appended to any `approvals.yaml`; the two existing ledgers stay as history and all 113 of their entries still resolve. Scenario documents no longer carry an approval status — "draft" and "pending approval" stop being meaningful states for them. **What this gives up:** the 2026-08-26 failure mode — one dispatch writing scenario, tests and code back to back with no independent eye — is no longer prevented by the gate. Pull-request review and CI actually executing the suites are what remain; neither is automatic, and the backend suites do not currently run in CI. Full statement in `docs/architecture/testing-strategy.md`.
