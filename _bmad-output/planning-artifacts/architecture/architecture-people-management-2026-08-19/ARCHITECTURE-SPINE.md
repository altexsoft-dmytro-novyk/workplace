...
- **Amended 2026-09-04 (approval requirement removed; clarified 2026-09-12):** the **ordering** above stands — scenario, then red Stage-2 test, then production written until it passes — but the **approval gate does not.** No stage waits on a developer sign-off, a dispatch may span more than one stage, and the 2026-08-26 extension above is withdrawn as a binding rule (kept here as the record of why it once existed). `approvals` entries are preserved as read-only history; they cannot authorize, block, or retroactively validate a stage transition. A ledger may append a separately typed `ratifications` record only to classify an independently reviewed, already-completed out-of-order dispatch. That record must name the exact artifact and commit, the original author, an independent ratifier, the decision time, evidence and rationale, and disposition `recorded-late-not-gated`; it records non-compliance and never makes the dispatch a clean gate pass. Scenario documents no longer carry an approval status — "draft" and "pending approval" stop being meaningful states for them. **What this gives up:** the 2026-08-26 failure mode — one dispatch writing scenario, tests and code back to back with no independent eye — is no longer prevented by the gate. Pull-request review and CI actually executing the suites are what remain; neither is automatic, and the backend suites do not currently run in CI. Full statement in `docs/architecture/testing-strategy.md`.
