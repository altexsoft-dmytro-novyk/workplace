...
- **Persisted approval rule:** every AD-1 stage approval is recorded in the append-only ledger `_bmad-output/specs/spec-access-control-kernel-mvp/approvals.yaml` as `story_id`, `stage`, `repo`, `artifact_path`, `commit`, `author`, `approver`, `decision`, and `timestamp`. `repo` is `workspace` or `services/backend`, with `artifact_path` relative to that repository's root: the workspace spans two git repositories, so a bare revision cannot identify which one and the record would not be resolvable. `author` and `approver` must differ. A dispatch may not start until the prior stage's record exists **and** verifies — the commit resolves in the named repository and the artifact is present at that revision. A prose assertion that a stage was approved is not an approval; the verifiable record is.
