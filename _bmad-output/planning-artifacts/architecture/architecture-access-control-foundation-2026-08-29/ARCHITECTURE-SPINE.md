...
- **Persisted approval rule — WITHDRAWN 2026-09-04; clarified 2026-09-12.** The former rule required every AD-1 stage approval to be recorded in the append-only ledger `_bmad-output/specs/spec-access-control-kernel-mvp/approvals.yaml` (`story_id`, `stage`, `repo`, `artifact_path`, `commit`, `author`, `approver`, `decision`, `timestamp`), with `author` and `approver` differing, and blocked a dispatch until the prior stage's record existed and verified. **AD-1 no longer requires stage approval**, so those historical `approvals` entries are not appended, edited, or used to block a dispatch. A separately typed `ratifications` record may be appended only to classify independently reviewed out-of-order work already completed: it names the artifact and resolvable commit, original author, independent ratifier, decision time, evidence and rationale, and has disposition `recorded-late-not-gated`. It records non-compliance; it is never a clean gate pass or a stage-transition authorization. The `repo` field's rationale remains true of any two-repository record: the workspace spans two git repositories, so a bare revision cannot identify which one. Full statement of the change in `docs/architecture/testing-strategy.md`.
...
| **Historical, verifiable** | Withdrawn AD-1 approvals | `approvals.yaml` preserves legacy records. A separately typed late-ratification record must identify an independent reviewer, resolvable commit/artifact, evidence and rationale; it records non-compliance and never authorizes a stage. |
...
| **Orders each new production behavior** | AD-1 sequence | Scenario, committed-red kernel integration evidence, then production; no approval ledger entry blocks a stage. |
